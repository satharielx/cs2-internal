...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            char m_hMyWearables[0x18]; // 0x1350            
            // metadata: MNotSaved
            source2sdk::modellib::AttachmentHandle_t m_leftFootAttachment; // 0x1368            
            // metadata: MNotSaved
            source2sdk::modellib::AttachmentHandle_t m_rightFootAttachment; // 0x1369            
            uint8_t _pad136a[0x2]; // 0x136a
            // metadata: MNotSaved
            source2sdk::client::C_BaseCombatCharacter_WaterWakeMode_t m_nWaterWakeMode; // 0x136c            
            // metadata: MNotSaved
            float m_flWaterWorldZ; // 0x1370            
            // metadata: MNotSaved
            float m_flWaterNextTraceTime; // 0x1374            
            uint8_t _pad1378[0x60];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in C_BaseCombatCharacter because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::C_BaseCombatCharacter) == 0x13d8);
    };
};
