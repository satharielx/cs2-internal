...
HRESULT __stdcall hkPresent(IDXGISwapChain* swap_chain, UINT sync_interval, UINT flags) {
    ...
    if (!s_stopping && swap_chain && !(flags & DXGI_PRESENT_TEST)) {
        ...
        if (!s_stopping && s_resizes_in_progress == 0)
...
namespace hooks {
    ...
    void create() {
        ...
        try {
            // Create Present hook
            void* present_target = sdk::find_pattern("gameoverlayrender64.dll",
            ...
            void* resize_target = sdk::find_pattern("gameoverlayrender64.dll",
...

            {
                ...
                if (subtick_addr) {
                    ...
                    if (status == MH_OK) {
                        MH_EnableHook(subtick_addr);
                        debug_console::Console::Get().Success("[HOOK] SubTickAngle hooked at 0x%llX", (uintptr_t)subtick_addr);
